...
uses
  ...
  TFrmViabilidadPtP = class(TForm)
    ...
    procedure SBActualizaClick(Sender: TObject);
  private
...
var
  FrmViabilidadPtP: TFrmViabilidadPtP;

implementation

{$R *.dfm}

uses WndSeleccionaAntena;

function TFrmViabilidadPtP.InterpolateHeight(A, B: TPoint3D; Lat, Lon: Double): Double;
var
  t: Double;
begin
...
function TFrmViabilidadPtP.DegToRad(Degrees: Double): Double;
begin
  Result := Degrees * Pi / 180;
end;

function TFrmViabilidadPtP.Haversine(lat1, lon1, lat2, lon2: Double): Double;
var
...
function TFrmViabilidadPtP.FresnelZoneRadius2(lat1, lon1, lat2, lon2, latMid, lonMid, freq: Double): Double;
var
...
procedure TFrmViabilidadPtP.LlenaTabla;
Const
...
procedure TFrmViabilidadPtP.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := caFree;
end;

function TFrmViabilidadPtP.FresnelZoneRadius(x1, y1, x2, y2, x, y, frequencyMHz: Double): Double;
const
...
function TFrmViabilidadPtP.CalculateYForX(x1, y1, x2, y2, x: Double): Double;
var
  m, b, y: Double;
begin
...
procedure TFrmViabilidadPtP.MapaChangeMapZoom(Sender: TObject);
//var
//  x: Integer;
begin
...
function TFrmViabilidadPtP.Max(Number1, Number2: double): double;
begin
...
procedure TFrmViabilidadPtP.SBAntenaBClick(Sender: TObject);
var wnd: TFrmSeleccionaAntena;
begin
  wnd := TFrmSeleccionaAntena.Create(Self);
  wnd.Padre := Self;
  wnd.ShowModal;
end;

procedure TFrmViabilidadPtP.SBAntenaClick(Sender: TObject);
var
...
procedure TFrmViabilidadPtP.SetPoint(lat: double; lng: double; cam: string; aColor: TColor);
begin
...
procedure TFrmViabilidadPtP.LoadDataFromJSON(const JSONStr: string);
var
 ...
 ***********************************************************************)
procedure TFrmViabilidadPtP.SkPaintBox1Draw(ASender: TObject;
...
procedure TFrmViabilidadPtP.SpeedButton1Click(Sender: TObject);
var
...
procedure TFrmViabilidadPtP.SpeedButton2Click(Sender: TObject);
begin
  ...
  begin
    try
      ...
      try
        ...
        // Distancia en mapa
        LabelDistancia.Caption := Line.Distance.ToString + ' km.';

        // Habilita SpeedButton
        SBActualiza.Enabled := True;
...
procedure TFrmViabilidadPtP.SBActualizaClick(Sender: TObject);
begin
...
procedure TFrmViabilidadPtP.StringGrid1DrawCell(Sender: TObject; ACol, ARow: LongInt;
  Rect: TRect; State: TGridDrawState);
var
...
procedure TFrmViabilidadPtP.StringGrid1SelectCell(Sender: TObject; ACol,
  ARow: LongInt; var CanSelect: Boolean);
begin
  TrackBar1.Position := ARow-1;
end;

procedure TFrmViabilidadPtP.TBZoomChange(Sender: TObject);
begin
  Label14.Caption := TBZoom.Position.ToString;
  Mapa.Zoom := TBZoom.Position;
end;

procedure TFrmViabilidadPtP.TrackBar1Change(Sender: TObject);
begin
...
procedure TFrmViabilidadPtP.LoadAndConvertJSON(const FileName: string);
var
...
function TFrmViabilidadPtP.ConvertJSON(const JSONStr: string): string;
var
...
function TFrmViabilidadPtP.CalcularTiltCompensado(
  LatA, LonA, AltA: Double;   // Coordenadas y altura de la antena A
  LatB, LonB, AltB: Double;   // Coordenadas y altura de la antena B
  TiltB: Double                // Tilt actual de la antena B en grados
): Double;
...
function TFrmViabilidadPtP.CalcularAzimutCompensado(
  LatA, LonA: Double;  // Latitud y longitud de la antena A
  LatB, LonB: Double;  // Latitud y longitud de la antena B
  AzimutB: Double      // Azimut actual de la antena B en grados
): Double;
...
function TFrmViabilidadPtP.CalcularAzimut(LatA, LonA, LatB, LonB: Double): Double;
var
...
function TFrmViabilidadPtP.CalcularTilt(LatA, LonA, AltA, LatB, LonB, AltB: Double): Double;
var
